...
namespace LongArithm {
    LongNumber::LongNumber() : sign(false), digitsAfterDecimal(0), base(2), chunks(std::vector<uint32_t>()) {}
        
    inline int LongNumber::charToDigit(const char c) const {
        int digit = 0;
        if (isdigit(c)) {
    ...
    inline int LongNumber::getDigitsPerChunk(void) const {
        return std::floor(32 / std::log2(base));
    }

    void LongNumber::parseFractionPart(std::string input, int base, int digitsPerChunk){
    ...
    void LongNumber::parseWholePart(std::string input, int base, int digitsPerChunk) {
    ...
    LongNumber LongNumber::fromString(std::string input, uint32_t digitsAfterDecimal, int base = 2) {
    ...
    const std::string LongArithm::LongNumber::toString(void) const {
        ...
        for (int i = chunks.size() - 1; i > fractionChunks - 1; i--) {
            uint32_t chunk = chunks[i];
            std::string chunkStr;
            while (chunk > 0) {
                int digit = chunk % base;
                chunkStr += digitToChar(digit);
                chunk /= base;
            };
        ...
        if (fractionChunks == 0) return output;

        output += '.';
        uint64_t remainder = 0;
        uint64_t basePower = 1ULL << 32;  // 2^32
        int digitsPerChunk = getDigitsPerChunk();
        ...
        for (size_t i = 0; i < fractionChunks; i++) {
            ...
            int j = 0;
            while (j < digitsPerChunk && i * digitsPerChunk + j < digitsAfterDecimal) {
                remainder *= base;
    ...
    // Debug purposes only
    void LongNumber::printChunks(void) {
        int fractionChunks = getFractionChunks();
